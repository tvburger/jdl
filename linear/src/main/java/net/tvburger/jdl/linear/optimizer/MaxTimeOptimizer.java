...
/**
 * Custom optimizer that implements Max Time optimizer. It walks the max seen gradient in the same direction,
 * rather than the last gradient.
 *
 * @param <N>
 */
public class MaxTimeOptimizer<N extends Number> implements LinearModelOptimizer<N>, GradientDescentOptimizer<N> {
    ...
    public MaxTimeOptimizer(JavaNumberTypeSupport<N> typeSupport) {
    ...
    public void setOptimalWeights(LinearBasisFunctionModel<N> regression, DataSet<N> trainSet) {
        ...
        N[] maxGradients = typeSupport.createArray(regression.getParameterCount());
        ...
        for (int e = 1; e <= epochs; e++) {
            ...
            for (int i = 0; i < maxGradients.length; i++) {
                N newGradient = adjustment.get(i + 1);
                if (!typeSupport.sameSign(newGradient, maxGradients[i]) || typeSupport.greaterThan(typeSupport.absolute(newGradient), typeSupport.absolute(maxGradients[i]))) {
                    maxGradients[i] = newGradient;
                }
            }

            Vector<N> adjustmentWithAccumulatedGradients = Vectors.of(typeSupport, maxGradients).transpose();
    ...
    private N bounded(N value) {
        if (typeSupport.greaterThan(value, typeSupport.one())) {
            return typeSupport.one();
        } else if (typeSupport.greaterThan(typeSupport.minusOne(), value)) {
            return typeSupport.minusOne();
        }
        return value;
    }
