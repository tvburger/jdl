 ...
 * @param <N>
 */
public class MultiArgOptimizer<N extends Number> implements LinearModelOptimizer<N>, GradientDescentOptimizer<N> {
    ...
    public MultiArgOptimizer(JavaNumberTypeSupport<N> typeSupport) {
    ...
    public void setOptimalWeights(LinearBasisFunctionModel<N> regression, DataSet<N> trainSet) {
        ...
        N[] accumulatedGradients = typeSupport.createArray(regression.getParameterCount());
        ...
        for (int e = 1; e <= epochs; e++) {
            ...
            for (int i = 0; i < accumulatedGradients.length; i++) {
                ...
                if (typeSupport.sameSign(newGradient, accumulatedGradients[i])) {
                    multiplier[i] = typeSupport.multiply(multiplier[i], gamma);
                    accumulatedGradients[i] = bounded(typeSupport.multiply(typeSupport.add(accumulatedGradients[i], newGradient), multiplier[i]));
                } else {
            ...
            Vector<N> adjustmentWithAccumulatedGradients = Vectors.of(typeSupport, accumulatedGradients).transpose();
