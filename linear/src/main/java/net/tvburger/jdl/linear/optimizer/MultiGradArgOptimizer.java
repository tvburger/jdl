 ...
 * @param <N>
 */
public class MultiGradArgOptimizer<N extends Number> implements LinearModelOptimizer<N>, GradientDescentOptimizer<N> {
    ...
    public MultiGradArgOptimizer(JavaNumberTypeSupport<N> typeSupport) {
        ...
        for (int e = 1; e <= epochs; e++) {
            ...
            for (int i = 0; i < accumulatedGradients.length; i++) {
                ...
                if (typeSupport.sameSign(newGradient, accumulatedGradients[i])) {
                    ...
                    accumulatedGradients[i] = typeSupport.add(accumulatedGradients[i], typeSupport.multiply(newGradient, multiplier[i]));
                } else {
